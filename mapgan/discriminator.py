...
def discriminator(X, current_lod, alpha, dim=64):
    with tf.variable_scope('discriminator', reuse=tf.AUTO_REUSE):
        x = X
        tf.summary.histogram('x', x)
        x, downsample = layer(None, x, 4, current_lod, alpha, dim, dim, bypass=False) # 32 x 32
        x, downsample = layer(x, downsample, 3, current_lod, alpha, dim, 2 * dim, bypass=False) # 16 x 16
        x, downsample = layer(x, downsample, 2, current_lod, alpha, 2 * dim, 3 * dim, bypass=False) # 8 x 8
        x, downsample = layer(x, downsample, 1, current_lod, alpha, 3 * dim, 3 * dim) # 4 x 4
        x, _          = layer(x, downsample, 0, current_lod, alpha, 3 * dim, 3 * dim) # 4 x 4
