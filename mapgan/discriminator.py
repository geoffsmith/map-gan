...
def discriminator(X, current_lod, alpha):
    ...
    with tf.variable_scope('discriminator', reuse=tf.AUTO_REUSE):
        ...
        x, downsample = layer(x, downsample, 2, current_lod, alpha, 1 * dim, 4 * dim, bypass=False) # 16 x 16
        x, downsample = layer(x, downsample, 1, current_lod, alpha, 4 * dim, 8 * dim, bypass=False) # 8 x 8 
